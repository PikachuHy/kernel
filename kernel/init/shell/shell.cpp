...
constexpr int SYS_SERIAL_READ    = 54;
...
static auto p(const char* m) -> void {
    syscall6(SYS_DEBUG_PRINT, (uint64_t)m, 0, 0, 0, 0);
}

static auto pd(uint64_t n) -> void {
    if (!n) { p("0"); return; }
...
static auto cw(uint32_t h, const void* d, size_t n) -> int {
    sw.d = d; sw.sz = n; sw.hnd = nullptr; sw.n = 0;
    return (int)syscall6(SYS_CHANNEL_WRITE, h, (uint64_t)&sw, 0, 0, 0);
}

static auto cr(uint32_t h, void* b, size_t sz) -> int {
...
static auto cc(uint32_t h) -> void {
...
static auto getc() -> char {
    return (char)(syscall6(SYS_SERIAL_READ, 0, 0, 0, 0, 0) & 0xFF);
}

static auto pc(char c) -> void {
    char s[2] = {c, 0}; p(s);
}

static auto rl(char* buf, int mx) -> void {
    int i = 0;
    while (i < mx - 1) {
        char c = getc();
        if (c == '\r' || c == '\n') { pc('\n'); buf[i++] = '\n'; break; }
...
static auto fixpath(const char* in, char* out) -> void {
    int i = 0;
    if (!in || !in[0]) { out[0] = '/'; out[1] = 0; return; }
    if (in[0] == '/') { while (in[i]) { out[i] = in[i]; i++; } out[i] = 0; return; }
    out[0] = '/'; i = 1; int j = 0;
    while (in[j]) { out[i] = in[j]; i++; j++; }
    out[i] = 0;
}

static auto cmd_ls(const char* raw) -> void {
    ...
    for (uint32_t i = 0; i < cnt; i++) {
        p("  "); p(dd[i].type ? "[D]" : "[F]"); p(" "); p(dd[i].name); p(" ("); pd(dd[i].size); p(")\n");
    }
    cc(f);
}

static auto cmd_cat(const char* raw) -> void {
    char path[256]; fixpath(raw, path);
    uint32_t f = (uint32_t)syscall6(SYS_OPEN, (uint64_t)path, O_RDONLY, 0, 0, 0);
    if (f == 0 || f == 0xFFFFFFFF) { p("  nf\n"); return; }
    ...
    FileResponse* rp = (FileResponse*)rb;
    if (rp->result != 0) { cc(f); return; }
    for (uint64_t i = 0; i < rp->size && i < 64; i++) {
        uint8_t b = rb[sizeof(FileResponse) + i];
        if (b >= 32 && b < 127) { char c[2] = {(char)b, 0}; p(c); } else p(".");
    }
    p("\n"); cc(f);
}

static auto cmd_stat(const char* raw) -> void {
    char path[256]; fixpath(raw, path);
    uint32_t f = (uint32_t)syscall6(SYS_OPEN, (uint64_t)path, O_RDONLY, 0, 0, 0);
    if (f == 0 || f == 0xFFFFFFFF) { p("  nf\n"); return; }
