...
extern "C" void _start() {
    ...
    if (fh != 0 && fh != 0xFFFFFFFF) {
        print("  [3] /kernel.elf opened OK, sending Read(0,4)...\n");
        FileMsg msg = {FileMsg::Read, 0, 0, 4};
        int wrc = channel_write((uint32_t)fh, &msg, sizeof(msg));
        if (wrc == 0) {
            print("  [4] waiting for response...\n");
            uint8_t rbuf[sizeof(FileResponse) + 4];
            int rrc = channel_read((uint32_t)fh, rbuf, sizeof(rbuf));
            if (rrc >= (int)sizeof(FileResponse)) {
                FileResponse* resp = (FileResponse*)rbuf;
                if (resp->result == 0 && resp->size >= 4) {
                    uint8_t* d = rbuf + sizeof(FileResponse);
                    print("  [5] ELF magic: ");
                    print_hex(d[0]); print(" "); print_hex(d[1]); print(" ");
                    print_hex(d[2]); print(" "); print_hex(d[3]); print(" -> ");
                    print(d[0]==0x7F&&d[1]=='E'&&d[2]=='L'&&d[3]=='F'?"YES\n":"NO\n");
                } else { print("  [5] read error result="); print_hex(resp->result); print("\n"); }
            } else { print("  [5] ch_read failed rc="); print_hex((uint64_t)rrc); print("\n"); }
        } else { print("  [4] ch_write failed\n"); }
    } else {
