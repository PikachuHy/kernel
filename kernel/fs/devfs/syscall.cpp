...
uint64_t sys_channel_write(uint64_t a1, uint64_t a2, uint64_t, uint64_t) {
    ...
    int ret = static_cast<Channel*>(obj)->Write(
        args->data, args->data_len, args->handles, args->num_handles, endpoint_b);
    // Trace IPC for file read debugging (only handles >= 2)
    if (h >= 2) { klog(proc->name); klog(":W h="); klog_hex(h);
                  klog(" ep="); klog(endpoint_b?"B":"A");
                  klog(" len="); klog_hex(args->data_len);
                  klog(" ret="); klog_hex(ret); klog("\n"); }
    return ret;
}

uint64_t sys_channel_read(uint64_t a1, uint64_t a2, uint64_t a3, uint64_t) {
    Process* proc = current_process();
    if (!proc) { klog("CR:no-proc\n"); return static_cast<uint64_t>(-1); }

    handle_t h = static_cast<handle_t>(a1);
    Rights existing;
    KernelObject* obj = proc->handles.Lookup(h, Rights{.mask = Rights::Read}, &existing);
    if (!obj || obj->type() != KernelObject::Type::Channel) {
        klog(proc->name); klog(":R bad-h="); klog_hex(h); klog("\n");
        return static_cast<uint64_t>(-1);
    }
    ...
    if (h >= 2) { klog(proc->name); klog(":R h="); klog_hex(h);
                  klog(" rc="); klog_hex(rc);
                  klog(" len="); klog_hex(out_len); klog("\n"); }
